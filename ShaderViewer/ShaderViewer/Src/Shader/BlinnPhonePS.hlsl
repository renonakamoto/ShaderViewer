...
float BlinnPhong(float4 wp, float3 nor, float3 lig)
{
    float3 view_dir = normalize(wp - CameraPos);
    float3 half_vec = normalize(lig + -view_dir);
    float  specular  = pow(saturate(dot(nor, half_vec)), Shininess);
...
float4 ps_main(PS_IN input) : SV_Target
{
   ...
   // スペキュラーカラー
    float4 specular_color = BlinnPhong(input.posw, input.norw, input.light);
