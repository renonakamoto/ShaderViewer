...
bool SoundManager::LoadBGMData(std::string fileName_, std::string keyName_)
{
	LPDIRECTSOUNDBUFFER8 sound_buf;
	if (LoadData(fileName_, sound_buf) == false)
	{
		return false;
	}

	m_BGMData.emplace(keyName_, sound_buf);
...
bool SoundManager::LoadSEData(std::string fileName_, std::string keyName_)
{
	...
	sound_data.m_SoundData = sound_buf;
	m_SEData.emplace(keyName_, sound_data);
...
bool SoundManager::LoadData(std::string fileName_, LPDIRECTSOUNDBUFFER8& outData_)
{
	...
	if (FAILED(pDSBuf->QueryInterface(IID_IDirectSoundBuffer8, (LPVOID*)outData_)))
	{
		return false;
	}
	...
	DWORD Bufsize[2] = { 0 };
	if (FAILED(outData_->Lock(0, dataCK.cksize, &pBuf[0], &Bufsize[0], &pBuf[1], &Bufsize[1], 0)))
	{
		return false;
	}
	...
	outData_->Unlock(&pBuf[0], Bufsize[0], &pBuf[1], Bufsize[1]);
...
{
	return HasKey(key_, m_BGMData);

}

bool SoundManager::HasKeySE(std::string key_)
{
	return HasKey(key_, m_SEData);
}
